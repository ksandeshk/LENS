...
class IntegratedGradientsAttack(object):
    ...
    def create_attack_ops(self):
        ...
        finalp = []
        for j in elem1:
                finalp = finalp + submat(j)
        finalp = np.array(finalp)  
        finalp= finalp[finalp < 16384]
        fp = np.unique(np.array(finalp))
        ## new metric end
    ...
    def return_perturb(self, in_image, pert, alpha, bound=8.0):

        out_image = np.clip(in_image + alpha * pert - self.test_image, -bound, bound)
        out_image = out_image.astype(np.int32)
        return out_image
    ...
    def iterative_attack(self):
        ...
        for i in range(self.attack_times):
            intersection, spearman, kendall, copy_orig_ig, copy_pert_ig, copy_image_pert, copy_pert  = self.iterative_attack_once()
            #intersection, spearman, kendall = self.iterative_attack_once()

            intersections.append(intersection)
            spearmans.append(spearman)
            kendalls.append(kendall)
            copy_orig_igs.append(copy_orig_ig)
            copy_pert_igs.append(copy_pert_ig)
            copy_image_perts.append(copy_image_pert)
            copy_perts.append(copy_pert)

        return np.array(intersections), np.array(spearmans), np.array(kendalls), np.array(copy_orig_igs), np.array(copy_pert_igs), np.array(copy_image_perts), np.array(copy_perts)
    ...
    def iterative_attack_once(self):
        ...
        for counter in range(self.attack_iters):
            ...
            pert_added = self.return_perturb(test_image_pert, pert, self.alpha, self.epsilon)
        ...
        if min_criterion==1.:
            ...
            #return 1., 1., 1.
            return 1., 1., 1., self.saliency1_flatten, self.saliency1_flatten, self.test_image, pert_added
        ...
        return intersection, spearman, kendall, origin_ig, perturbed_ig, test_image_pert, pert_added
